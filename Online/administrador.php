                                    ...
                                    if (isset($_REQUEST['nuevo'])) {                                             
                                        switch ($tabla) {
                                            ...
                                            include ("static/estado.php");    
                                                                    //-------- exit -------
                                            $query = "SELECT codigo, nombre, marca, nota, valor, estado, cantidad, imagen FROM producto";
                                            $result = mysql_query($query) or die("error". mysql_error());
                                            $a = mysql_num_fields($result);
                                            while ($row = mysql_fetch_array($result)) {
                                                for ($i=0; $i < $a ; $i++) { 
                                            ...
                                            mysql_query("insert into categoria_estado values('','$estado')");}else{
                                                if($tabla=='categoria_producto'){
                                                    mysql_query("insert into categoria_producto values('','$categoria')");}else{
                                                        if($tabla=='producto'){
                                                            echo $_POST['idcatest'];
                                                            mysql_query("insert into producto values ('','$idcat','$idest','$idcatest', '$codigo', '$nombre', '$marca', '$nota', '$valor', '$estado', '$cantidad', '$imagen')");}else{
                            ...
                            $("#cont").change(function() {
                                //if ($("#cont option[value='3']").attr('selected')) {
                                    ...
                                    if ($var[0] == 5) {
                                        ...
                                        if (isset($_REQUEST['guardar'])) {
                                            echo $_POST['idcatest'];
                                            mysql_query("insert into producto values ('','$idcat','$idest','$idcatest', '$codigo', '$nombre', '$marca', '$nota', '$valor', '$estado', '$cantidad', '$imagen')");
